...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor>
{
    ...
    public boolean eBalanceada() {
        return eBalanceadaRecursivo(this.raiz) != -1;
    }

    private int eBalanceadaRecursivo(No no) {
        if (no == null) {
            return 0;
        }

        int altEsq = eBalanceadaRecursivo(no.esq);

        if (altEsq == -1) {
            return -1;
        }

        int altDir = eBalanceadaRecursivo(no.dir);

        if (altDir == -1) {
            return -1;
        }

        int difAbsoluta = Math.abs(altEsq - altDir);
        if (difAbsoluta > 1) {
            return -1;
        }
