...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor>
{
    ...
    public No obtemAncestralComum(No primeiroNo, No segundoNo){
        return obtemAncestralComumRecursivo(raiz, primeiroNo, segundoNo);
    }

    private No obtemAncestralComumRecursivo(No no, No primeiroNo, No segundoNo){
        if(no == null){
            return null;
        }
        if(primeiroNo.chave.compareTo(no.chave) < 0 && segundoNo.chave.compareTo(no.chave) < 0){
            return obtemAncestralComumRecursivo(no.esq, primeiroNo, segundoNo);
        }
        else if( primeiroNo.chave.compareTo(no.chave) > 0 && segundoNo.chave.compareTo(no.chave) > 0){
            return obtemAncestralComumRecursivo(no.dir, primeiroNo, segundoNo);
        }
        else
            return no;
    }
