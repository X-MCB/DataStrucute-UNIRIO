...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor>
{
    ...
    //Metodo auxiliar para construir arvore
    public void constroiArvore(int tam){
        raiz = null;
        Random random = new Random();
        int valor = random.nextInt(100); // Gerando valores aleatorios de 0 a 99

        for(int i = 0; i < tam; i++){
            Integer num = valor;
            raiz = put(raiz, (Chave) num, (Valor) num);
            valor = random.nextInt(100); // Gerando valores aleatorios de 0 a 99
        }
    }
    public boolean removeForaIntervalo(Chave chaveMin, Chave chaveMax){
        ...
        String raiz2 = raiz.toString();
        raiz = removeForaIntervaloRecursivo(raiz, chaveMin, chaveMax);
        ...
        return !raiz2.equals(raiz3);
    }

    private No removeForaIntervaloRecursivo(No no, Chave chaveMin, Chave chaveMax){
        if( no == null){
            return null;
        }
        if(no.chave.compareTo(chaveMin) < 0 || no.chave.compareTo(chaveMax) > 0){
            no = delete(no, no.chave);
            return removeForaIntervaloRecursivo(no, chaveMin, chaveMax);
        }

        no.esq = removeForaIntervaloRecursivo(no.esq, chaveMin, chaveMax);
        no.dir = removeForaIntervaloRecursivo(no.dir, chaveMin, chaveMax);
    ...
    /* Remove o n� com o valor "val" da "�rvore" a partir do n� para o qual est� sendo cham
     */
    ...
    /**
     * Retorna o n�mero de n�s na �rvore cujas chaves s�o estritamente menores do que {@cod
     *
     * @param  chave a chave fornecida
     * @return o n�mero de n�s na �rvore cujas chaves s�o estritamente menores do que {@cod
     * @throws IllegalArgumentException se {@code chave} � {@code null}
     */
