...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    //METODO AUXILIAR PARA A QUESTAO 4 PARA RETORNAR EM TEMPO CONSTANTE O ULTIMO NO ACESSADO
    public Node ultimoNoAcessado(){
        return ultimoNo;
    }
    ...
    //ALTERADO PARA A QUESTAO 4
    //ALTERADO PARA A QUESTAO 4
    private Value get(Node x, Key key) {
        while (x != null) {
            int cmp = key.compareTo(x.key);
            ultimoNo = x;  // Mova a atribuição para fora das condições
            if (cmp < 0) {
                x = x.left;
            } else if (cmp > 0) {
                x = x.right;
    ...
    //IMprimir ultimo no acessado
    public String ImprimirUltimoNoAcessado() {
        if (ultimoNo != null) {
            return "Key: " + ultimoNo.key + ", Value: " + ultimoNo.val +
                    ", Color: " + (ultimoNo.color == RED ? "RED" : "BLACK");
        } else {
            return "Nenhum nó foi acessado ainda.";
        }
    }
    ...
    private Node put(Node h, Key key, Value val) {
        if (h == null) {
            ultimoNo = new Node(key, val, RED, 1);
            return ultimoNo;
        }
